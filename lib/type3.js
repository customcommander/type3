(function (root, factory) {
    ...
    type3 = {
        ...
        /**
         * Returns the parent of the first text node matching given pattern within given node.
         *
         * Returns null if the pattern wasn't found.
         *
         * @method queryTextParent
         * @param node {Element} The node to look into.
         * @param pattern {String|RegExp} The pattern to look for.
         * @return {Element}
         */
        queryTextParent: checkArguments(function (node, pattern) {
            var res = findTextNodes(node, pattern, true);
            return res.length === 1 ? res[0].parentNode : null;
        }),

        /**
         * Returns an array of parents of text nodes matching given pattern within given node.
         *
         * Returns an empty array if the pattern wasn't found.
         *
         * @method queryTextParentAll
         * @param node {Element} The node to look into.
         * @param pattern {String|RegExp} The pattern to look for.
         * @return {Element[]}
         */
        queryTextParentAll: checkArguments(function (node, pattern) {
            var res = findTextNodes(node, pattern);
            return res.map(function (txtNode) {
                return txtNode.parentNode;
            });
        })
    };
